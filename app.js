...
function render(){const q=$("searchInput").value.trim().toUpperCase();if(!q){$("results").innerHTML='<div class="empty">品番・商品名を入力してください。</div>';return}let m=products.filter(p=>p.sku.toUpperCase().includes(q)||txt(p.name).toUpperCase().includes(q));m=m.slice(0,C.MAX_RESULTS||200);$("results").innerHTML=m.length?m.map(p=>`<article class="item"><div class="item-head"><div><div class="sku">${esc(p.sku)}</div><div class="name">${esc(p.name)}</div></div><div class="jan">JAN ${esc(p.jan)}</div></div><div class="table-wrap"><table><thead><tr>${stores.map(s=>`<th>${esc(s.replace("ららぽーと",""))}</th>`).join("")}</tr></thead><tbody><tr>${stores.map(s=>{const x=cur(p,s);return `<td class="${x.current>0?"":"zero"}"><div class="current">${x.current}</div><div class="detail">月曜 ${x.base} − 売れ ${x.sold}</div></td>`}).join("")}</tr></tbody></table></div></article>`).join(""):'<div class="empty">一致する品番・商品名がありません。</div>'}
