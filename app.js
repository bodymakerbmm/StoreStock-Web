...
function matchesProductSearch(product, query){
  ...
  if(!q)return true;
  const fields=[product?.sku,product?.品番,product?.productName,product?.商品名,product?.name,product?.jan,product?.JAN];
  return fields.some(v=>txt(v).toLowerCase().includes(q));
}
